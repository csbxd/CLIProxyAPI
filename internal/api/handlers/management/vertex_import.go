...
func (h *Handler) ImportVertexCredential(c *web.Context) {
	if h == nil || h.cfg == nil {
		c.JSON(http.StatusServiceUnavailable, web.H{"error": "config unavailable"})
		return
	}
	if h.cfg.AuthDir == "" {
		c.JSON(http.StatusServiceUnavailable, web.H{"error": "auth directory not configured"})
		return
	}

	fileHeader, err := c.FormFile("file")
	if err != nil {
		c.JSON(http.StatusBadRequest, web.H{"error": "file required"})
		return
	}

	file, err := fileHeader.Open()
	if err != nil {
		c.JSON(http.StatusBadRequest, web.H{"error": fmt.Sprintf("failed to read file: %v", err)})
		return
	}
	defer file.Close()
	...
	if err := json.Unmarshal(data, &serviceAccount); err != nil {
		c.JSON(http.StatusBadRequest, web.H{"error": "invalid json", "message": err.Error()})
		return
	}

	normalizedSA, err := vertex.NormalizeServiceAccountMap(serviceAccount)
	if err != nil {
		c.JSON(http.StatusBadRequest, web.H{"error": "invalid service account", "message": err.Error()})
		return
	}
	...
	if projectID == "" {
		c.JSON(http.StatusBadRequest, web.H{"error": "project_id missing"})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, web.H{"error": "save_failed", "message": err.Error()})
		return
	}
